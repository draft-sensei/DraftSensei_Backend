...
class IntelligentDraftRequest(BaseModel):
    ...
    banned_heroes: List[str] = Field(
        default_factory=list, description="List of banned hero names"
    )
    enemy_picks: List[str] = Field(
        default_factory=list, description="Enemy team's picked heroes"
    )
    ally_picks: List[str] = Field(
        default_factory=list, description="Your team's picked heroes"
    )
...
class HeroSuggestion(BaseModel):
    ...
    reasons: List[str] = Field(
        description="List of reasons why this hero is recommended"
    )
...
class IntelligentDraftResponse(BaseModel):
    ...
    suggestions: List[HeroSuggestion] = Field(
        description="Top 5 hero suggestions for this lane"
    )
