...
class MetaDraftEngine:
    ...
    def suggest_best_role_and_heroes(
        self, banned_heroes: List[str], enemy_picks: List[str], ally_picks: List[str]
    ) -> Dict[str, Any]:
        """
        ...
        suggestions = self.get_pick_suggestions(
            banned_heroes, enemy_picks, ally_picks, recommended_lane
        )
    ...
    def _get_meta_priority_lane(
        self, enemy_picks: List[str], banned_heroes: List[str]
    ) -> str:
        """
        ...
        if enemy_picks:
            ...
            if enemy_hero in self.heroes_data:
                enemy_lanes = (
                    self.heroes_data[enemy_hero]["meta"]
                    .get("roles", {})
                    .get("lane_priority", [])
                )
                if enemy_lanes:
    ...
    def _assess_composition_need(self, lane: str, ally_picks: List[str]) -> float:
        ...
        for ally_name in ally_picks:
            if ally_name in self.heroes_data:
                ...
                team_stats["tankiness"] += ally_meta.get("survivability", {}).get(
                    "tankiness", 0
                )
                team_stats["crowd_control"] += ally_meta.get("utility", {}).get(
                    "crowd_control", 0
                )
                team_stats["mobility"] += ally_meta.get("survivability", {}).get(
                    "mobility", 0
                )
    ...
    def _explain_lane_choice(
        ...
        for enemy_name in enemy_picks:
            if enemy_name in self.heroes_data:
                enemy_lanes = (
                    self.heroes_data[enemy_name]["meta"]
                    .get("roles", {})
                    .get("lane_priority", [])
                )
                if lane_name in enemy_lanes:
            ...
            scored_heroes.append(
                {
                    ...
                    "reasons": reasons,
                    "role": self.heroes_data[hero_name]["meta"]
                    .get("roles", {})
    ...
    def _calculate_team_composition_score(
        self, hero: Dict, ally_picks: List[str]
    ) -> float:
        """
        ...
        role_count = sum(
            ...
            for ally_name in ally_picks
            if ally_name in self.heroes_data
            and self.heroes_data[ally_name]["meta"].get("roles", {}).get("primary_role")
            == hero_role
        )
...
def get_pick_suggestions(
    ...
    """
    Convenience function for getting pick suggestion
