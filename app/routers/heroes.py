            ...
            for hero in all_heroes:
                ...
                if meta and isinstance(meta, dict):
                    if "attributes" in meta and "roles" in meta["attributes"]:
                        primary_role = meta["attributes"]["roles"].get(
                            "primary_role", ""
                        )
        ...
        for hero in heroes:
            ...
            if metadata and isinstance(metadata, dict):
                if "attributes" in metadata and "roles" in metadata["attributes"]:
                    primary_role = metadata["attributes"]["roles"].get(
                        "primary_role", "Unknown"
                    )
...
async def get_hero_by_id(hero_id: int, db: Session = Depends(get_db)) -> HeroSchema:
    """
...
async def get_hero_by_name(hero_name: str, db: Session = Depends(get_db)) -> HeroSchema:
    """
...
async def create_hero(
    hero_data: HeroCreate, db: Session = Depends(get_db)
) -> HeroSchema:
    """
    ...
    try:
        ...
        # Create new hero
        new_hero = Hero(name=hero_data.name, image=hero_data.image)
...
async def update_hero(
    hero_name: str, hero_update: HeroUpdate, db: Session = Depends(get_db)
) -> HeroSchema:
    """
...
async def bulk_update_heroes(
    bulk_update: BulkHeroUpdate, db: Session = Depends(get_db)
):
    """
    ...
    try:
        ...
        for hero_data in bulk_update.heroes:
            try:
                ...
                existing_hero = (
                    db.query(Hero).filter(Hero.name == hero_data.name).first()
                )
                ...
                if existing_hero:
                    ...
                    # Create new hero
                    new_hero = Hero(name=hero_data.name, image=hero_data.image or "")
...
async def delete_hero(hero_name: str, db: Session = Depends(get_db)):
    """
    ...
    try:
        ...
        for role, count in role_counts.items():
            distribution[role] = {
                "count": count,
                "percentage": (
                    round((count / total_heroes) * 100, 1) if total_heroes > 0 else 0
                ),
            }

        return {"total_heroes": total_heroes, "distribution": distribution}
